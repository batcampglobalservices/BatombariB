...
export default async function handler(req, res) {
  try {
    ...
    if (profile && (profile.education?.length > 0 || profile.experience?.length > 0)) {
      return res.status(200).json([
        { eduCards: profile.education || [] },
        { expCards: profile.experience || [] },
      ]);
    }

    return res.status(200).json(defaultBackground);
  } catch (error) {
